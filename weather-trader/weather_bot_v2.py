"""Weather Trading Bot v2 with position tracking and auto-exits."""
import requests
import re
...
def fetch_kalshi_markets(series: str):
    ...
    for m in resp.json().get("markets", []):
        ...
        if '<' in title:
            ...
            bucket_min, bucket_max = -999, int(match.group(1))
        elif '>' in title:
            match = re.search(r'>(\d+)°', title)
            bucket_min, bucket_max = int(match.group(1)), 999
        else:
...
def run_trading_cycle(cities: List[str] = None, budget: int = 20, 
    ...
    print("=" * 70)
    print("🌦️  WEATHER TRADING BOT v2")
    print("=" * 70)
        ...
        for pos in open_positions:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # Check if forecast still supports position
                    ...
                    elif pos.side == "yes" and current_price > 0.85 and (current_price - pos.entry_price) / pos.entry_price > 0.5:
                    ...
                    if exit_reason:
                        ...
                        if not dry_run:
                            ...
                            print(f"   🚀 EXIT EXECUTED: {result.get('order_id', 'pending')}")
                        else:
    ...
    for city in cities:
        ...
        for m in target_markets:
            ...
            print(f"\n   🎯 {m['ticker']}")
            print(f"      {m['title'][:60]}")
            print(f"      Forecast: {forecast_temp}°F | Bucket: {m['bucket_min']}-{m['bucket_max']}°F")
            print(f"      Edge: {edge:.0%} | Price: {price:.0%}")
            
            # Calculate position size
            kelly = min((edge * (1/price - 1)) / (1/price - 1), 0.25) if price > 0 else 0
            quantity = int((kelly * budget) / price) if price > 0 else 0
            
            if quantity < 1:
                print(f"      ⚠️  Quantity too small ({quantity})")
                continue
